...
pub fn speculative_test() {
  ...
  let assert Map(m2) = res2
  dict.get(m2, "name") |> should.equal(Ok(Single(Str("Alice"))))
  dict.get(m2, "balance") |> should.equal(Ok(Single(Int(100))))
  
  // 4. Verify original actor state doesn't have it
  let res1 = gleamdb.pull(db, fact.Uid(fact.EntityId(1)), [Wildcard])
  let assert Map(m1) = res1
