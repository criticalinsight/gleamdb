  ...
  case result {
    engine.Map(m) -> {
      case dict.get(m, "user/email") {
        Ok(engine.Single(Str("second@example.com"))) -> should.be_true(True)
        _ -> should.fail()
      }
    }
    _ -> should.fail()
  ...
  case result {
    engine.Map(m) -> {
      case dict.get(m, "user/role") {
        Ok(engine.Many(roles)) -> list.length(roles) |> should.equal(2)
        _ -> should.fail()
      }
    }
    _ -> should.fail()
...
pub fn latest_only_retention_test() {
  ...
  case history {
    [fact.Datom(_, _, Str("running"), _, _, fact.Assert)] -> should.be_true(True)
    _ -> should.fail()
  }
}
