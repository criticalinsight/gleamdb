...
pub fn pull_api_test() {
  ...
  let res2 = aarondb.pull(db, fact.Uid(fact.EntityId(1)), [Attr("name")])
  ...
  let res3 =
    aarondb.pull(db, fact.Uid(fact.EntityId(1)), [Nested("friend", [Wildcard])])
  ...
  // 4. Mixed pattern
  let res4 =
    aarondb.pull(db, fact.Uid(fact.EntityId(1)), [
      Attr("age"),
