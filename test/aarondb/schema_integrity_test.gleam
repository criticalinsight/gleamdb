...
pub fn cardinality_one_test() {
  ...
  // Assert first email
  let assert Ok(_) =
    aarondb.transact(db, [#(eid, "user/email", Str("first@example.com"))])

  // Assert second email (should replace first)
  let assert Ok(_) =
    aarondb.transact(db, [#(eid, "user/email", Str("second@example.com"))])

  // Pull current emails
  // Correct pattern usage: pull_attr returns List(PullItem)
  let result = aarondb.pull(db, eid, aarondb.pull_attr("user/email"))
...
pub fn cardinality_many_test() {
  ...
  let assert Ok(_) = aarondb.transact(db, [#(eid, "user/role", Str("admin"))])
  let assert Ok(_) = aarondb.transact(db, [#(eid, "user/role", Str("editor"))])
...
pub fn check_constraint_test() {
  ...
  aarondb.register_predicate(db, "is_positive", fn(val) {
    case val {
      Int(i) -> i > 0
      _ -> False
    }
  ...
  // Assert valid age
  let assert Ok(_) = aarondb.transact(db, [#(eid, "user/age", Int(25))])

  // Assert invalid age (should fail)
  let result = aarondb.transact(db, [#(eid, "user/age", Int(-5))])
...
pub fn latest_only_retention_test() {
  ...
  let assert Ok(_) =
    aarondb.transact(db, [#(eid, "system/status", Str("starting"))])
  let assert Ok(_) =
    aarondb.transact(db, [#(eid, "system/status", Str("running"))])

  // Pull history directly from engine to see datoms
  let history =
    aarondb.history(db, eid)
    |> list.filter(fn(d) { d.attribute == "system/status" })

  // With LatestOnly, the index prunes historical asserts.
  list.length(history) |> should.equal(1)
  case history {
    [fact.Datom(_, _, Str("running"), _, _, _, fact.Assert)] ->
      should.be_true(True)
    _ -> should.fail()
  }
}
