...
pub fn bm25_index_test() {
  ...
  { bm25.score(index, fact.EntityId(1), "hello", 1.2, 0.75) >. 0.0 }
  |> should.equal(True)
  { bm25.score(index, fact.EntityId(2), "hello", 1.2, 0.75) >. 0.0 }
  |> should.equal(True)

  // "gleam" appears in 2 and 3
  { bm25.score(index, fact.EntityId(2), "gleam", 1.2, 0.75) >. 0.0 }
  |> should.equal(True)
  { bm25.score(index, fact.EntityId(3), "gleam", 1.2, 0.75) >. 0.0 }
  |> should.equal(True)
