...
pub fn graph_traversal_test() {
  let assert Ok(db) = aarondb.start_named("graph_traverse_db", None)
  ...
  let assert Ok(_) = aarondb.transact(db, [f1, f2, f3, f4, f5, f6, f7, f8, f9])

  // 1. One hop: Alice's friends
  let assert Ok(friends) =
    aarondb.traverse(db, fact.uid(1), [Out("user/friends")], 5)
  ...
  list.length(likers) |> should.equal(1)
  // Just Dave (Entity 4)

  // 4. Max Depth Limit Rejection
  let res_err =
    aarondb.traverse(db, fact.uid(1), [Out("a"), Out("b"), Out("c")], 2)
