...
pub fn follower_becomes_candidate_on_timeout_test() {
  ...
  let #(new_state, effects) =
    raft.handle_message(state, raft.ElectionTimeout, self_pid)
...
pub fn candidate_increments_term_on_timeout_test() {
  let self_pid = process_extra.self()
  let peer1 = self_pid
  // Using self as fake peer for simplicity
  let state =
    raft.RaftState(
      role: raft.Candidate,
      current_term: 3,
...
pub fn follower_grants_vote_for_higher_term_test() {
  ...
  let has_granted =
    list.any(effects, fn(e) {
      case e {
        raft.SendVoteResponse(_, _, True) -> True
        _ -> False
      }
    })
...
pub fn follower_rejects_vote_for_stale_term_test() {
  let self_pid = process_extra.self()
  let candidate_pid = self_pid
  let state = raft.RaftState(..raft.new([]), current_term: 5)
  ...
  let has_rejected =
    list.any(effects, fn(e) {
      case e {
        raft.SendVoteResponse(_, _, False) -> True
        _ -> False
      }
    })
...
pub fn follower_resets_on_valid_heartbeat_test() {
  ...
  let state = raft.RaftState(..raft.new([]), current_term: 1)
...
pub fn leader_sends_heartbeats_on_tick_test() {
  ...
  let #(_new_state, effects) =
    raft.handle_message(state, raft.HeartbeatTick, self_pid)

  // Should send heartbeat to each peer
  let heartbeats =
    list.filter(effects, fn(e) {
      case e {
        raft.SendHeartbeat(_, _, _) -> True
        _ -> False
      }
    })
