...
pub fn composite_uniqueness_test() {
  ...
  let assert Ok(_) = aarondb.register_composite(db, ["user/first", "user/last"])
  ...
  res
  |> should.equal(Error(
    "Composite uniqueness violation: [\"user/first\", \"user/last\"]",
  ))
}
...
pub fn composite_missing_attr_test() {
  ...
  let assert Ok(_) =
    aarondb.transact(db, [#(fact.uid(1), "user/first", fact.Str("Rich"))])
  ...
  let assert Ok(_) =
    aarondb.transact(db, [#(fact.uid(2), "user/first", fact.Str("Rich"))])
}
...
pub fn composite_registration_guard_test() {
  ...
  let res = aarondb.register_composite(db, ["user/first", "user/last"])

  res |> should.be_error()
  res
  |> should.equal(Error(
    "Existing data violates new composite: [\"user/first\", \"user/last\"]",
  ))
}
