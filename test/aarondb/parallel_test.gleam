...
pub fn parallel_execution_test() {
  ...
  let facts =
    range(1, 600)
    |> list.map(fn(i) { #(fact.Uid(fact.EntityId(i)), "val", Int(i)) })
  ...
  let result =
    aarondb.query(db, [
      aarondb.p(#(types.Var("e"), "val", types.Var("v"))),
      types.Filter(types.Gt(types.Var("v"), types.Val(fact.Int(0)))),
      // Simple filter to force processing
    ])
