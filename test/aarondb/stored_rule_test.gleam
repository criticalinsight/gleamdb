...
fn grandparent_rule() -> types.Rule {
  types.Rule(head: #(types.Var("gp"), "grandparent", types.Var("gc")), body: [
...
pub fn stored_rule_test() {
  ...
  let assert Ok(_) = aarondb.store_rule(db, rule)
  ...
  let assert Ok(_) =
    aarondb.transact(db, [
      #(gp, "parent", Ref(EntityId(2))),
      // 1 is parent of 2
      #(p, "parent", Ref(EntityId(3))),
      // 2 is parent of 3
    ])
  ...
  let results = aarondb.query_with_rules(db, query, [])
...
pub fn rule_persistence_test() {
  ...
  // Since `aarondb.new()` uses ephemeral storage, we can't truly "restart" it easily 
  ...
  let query_rule = [
    types.Positive(#(types.Var("e"), "_rule/content", types.Var("content"))),
  ]
  let results = aarondb.query(db, query_rule)
  list.length(results.rows) |> should.equal(1)
}
