...
pub fn ordered_sequence_test() {
  ...
  let history =
    aarondb.history(db, fact.Uid(fact.EntityId(1)))
    |> list.filter(fn(d: fact.Datom) { d.attribute == "order" })
    |> list.sort(fn(a: fact.Datom, b: fact.Datom) {
      int.compare(a.tx_index, b.tx_index)
    })
...
pub fn fuzzy_json_test() {
  let noisy =
    "Some text before {\"key\": \"value\", \"nested\": {\"a\": 1}} some text after"
  utils.extract_json(noisy)
  |> should.equal("{\"key\": \"value\", \"nested\": {\"a\": 1}}")
...
pub fn composite_uniqueness_test() {
  ...
  let assert Ok(_) = aarondb.set_schema(db, "slug", config)
  let assert Ok(_) = aarondb.set_schema(db, "site_id", config)
  ...
  let assert Ok(_) = aarondb.transact(db, facts1)
  ...
  let assert Ok(_) = aarondb.transact(db, facts2)
  ...
  let result = aarondb.transact(db, facts3)
