...
pub fn columnar_aggregate_test() {
  ...
  let assert Ok(_) = aarondb.set_schema(db, "sensor/reading", config)

  // 2. Transact data
  let readings = [10, 20, 30, 40, 50]
  let facts =
    list.map(readings, fn(v) { #(fact.uid(1), "sensor/reading", fact.Int(v)) })
  ...
  let results_avg = aarondb.query(db, query_avg)
...
pub fn mixed_layout_test() {
  ...
  let assert Ok(_) = aarondb.set_schema(db, "val_col", col_config)
  ...
  let assert Ok(_) = aarondb.set_schema(db, "val_row", row_config)
