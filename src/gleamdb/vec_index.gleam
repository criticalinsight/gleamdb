...
pub fn insert(idx: VecIndex, entity: fact.EntityId, vec: List(Float)) -> VecIndex {
  ...
  case idx.entry_point {
    ...
    Ok(ep_id) -> {
      ...
      let #(final_layers, _) =
        int.range(from: level, to: -1, with: #(idx.layers, ep_for_level), run: fn(
          acc,
          i,
        ) {
          ...
          let ep_res = SearchResult(entity: curr_ep, score: vector.dot_product(vec, vec_ep))
          ...
          let neighbors =
            ...
            |> list.filter(fn(r) { r.entity != entity }) // Don't connect to self
