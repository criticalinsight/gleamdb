...
import aarondb/fact.{type Eid, Lookup, Uid}
import aarondb/shared/types.{
  type BodyClause, type DbState, type PullPattern, type PullResult,
  type QueryResult, PullMap,
}
...
pub type ShardedDb {
  ShardedDb(shards: Dict(Int, aarondb.Db), shard_count: Int, cluster_id: String)
}
...
pub fn start_local_sharded(
  ...
  |> list.each(fn(i) {
    process.spawn(fn() {
      let shard_cluster_id = cluster_id <> "_s" <> string.inspect(i)
      let res = case aarondb.start_named(shard_cluster_id, adapter) {
        Ok(db) -> Ok(#(i, db))
        Error(e) ->
          Error(
    ...
    _ -> {
      ...
      list.each(grouped_list, fn(pair) {
        ...
        process.spawn(fn() {
          ...
          let res = case aarondb.transact(shard_db, shard_facts) {
            Ok(state) -> Ok(state)
            Error(e) ->
              Error(
                "Shard "
...
pub fn transact_shard(
  ...
  case dict.get(db.shards, shard_id) {
    Ok(shard_db) -> aarondb.transact(shard_db, facts)
...
fn coordinate_reduce(
  ...
  aggregates: Dict(String, types.AggFunc),
        ...
        |> list.fold(base_row, fn(row_acc, agg_pair) {
          ...
          let final_val = case func {
            types.Sum | types.Count -> {
              ...
              aarondb_aggregate(shard_vals, types.Sum)
              |> result.unwrap(fact.Int(0))
            }
            types.Min ->
              aarondb_aggregate(shard_vals, types.Min)
              |> result.unwrap(fact.Int(0))
            types.Max ->
              aarondb_aggregate(shard_vals, types.Max)
              |> result.unwrap(fact.Int(0))
            _ -> {
...
pub fn pull(db: ShardedDb, eid: Eid, pattern: PullPattern) -> PullResult {
  ...
  int.range(
    ...
    run: fn(acc, _) {
      let res =
        process.receive(self, 5000) |> result.unwrap(PullMap(dict.new()))
