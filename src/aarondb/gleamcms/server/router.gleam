import aarondb
import aarondb/fact
import aarondb/gleamcms/ai/designer
...
pub fn handle_request(req: Request, db: aarondb.Db) -> Response {
    ...
    http.Post -> {
      ...
      case token {
        "sovereign-token-2026" ->
          ...
          |> wisp.set_header(
            "set-cookie",
            "gleamcms_token=sovereign-token-2026; Path=/; HttpOnly; SameSite=Strict",
          )
        _ ->
          wisp.response(200)
...
fn serve_static(req: Request, _file: List(String)) -> Response {
  let assert Ok(priv) = wisp.priv_directory("aarondb")
  use <- wisp.serve_static(
    req,
    under: "/static",
    from: priv <> "/static/gleamcms",
  )
  wisp.not_found()
}
...
fn serve_output(req: Request, _file: List(String)) -> Response {
  ...
  use <- wisp.serve_static(
    ...
    from: "/Users/brixelectronics/Documents/mac/gswarm/gleamcms_output",
  )
  wisp.not_found()
}
...
fn handle_generate(req: Request, db: aarondb.Db) -> Response {
  case req.method {
    http.Post -> {
      let theme_param =
        wisp.get_query(req)
  ...
  case req.method {
    http.Post -> {
      ...
      case prompt {
        ...
        _ -> {
          let project_id =
            designer.get_env("GOOGLE_PROJECT_ID") |> result.unwrap("")
          let bearer_token =
            designer.get_env("GOOGLE_BEARER_TOKEN") |> result.unwrap("")

          case project_id, bearer_token {
            "", _ ->
              wisp.internal_server_error()
              |> wisp.json_body("{\"error\": \"GOOGLE_PROJECT_ID not set\"}")
            _, "" ->
              wisp.internal_server_error()
              |> wisp.json_body("{\"error\": \"GOOGLE_BEARER_TOKEN not set\"}")
            pid, token -> {
...
fn handle_sync(req: Request, db: aarondb.Db) -> Response {
  case req.method {
    http.Post -> {
      use body <- wisp.require_string_body(req)
