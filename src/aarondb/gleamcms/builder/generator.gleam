import aarondb
import aarondb/gleamcms/builder/theme as theme_provider
import aarondb/gleamcms/db/post.{type Post, Published}
...
pub fn build(db: aarondb.Db, theme_name: String) -> BuildReport {
...
pub fn build_all(db: aarondb.Db) -> List(BuildReport) {
  library.get_configs()
  |> list.map(fn(c) { build(db, c.name) })
}

/// Specialized build: 50 sites, each with exactly ONE unique post.
pub fn build_showcase(db: aarondb.Db) -> List(BuildReport) {
  ...
  list.index_map(configs, fn(config, i) {
    ...
    let theme_posts = case list.drop(showcase_posts, i) |> list.first {
      Ok(p) -> [p]
      Error(_) ->
        case list.first(showcase_posts) {
          Ok(p) -> [p]
          Error(_) -> []
        }
    }
