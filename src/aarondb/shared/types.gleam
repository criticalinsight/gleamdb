...
pub type IndexQuery {
  ...
  Custom(data: String)
  // Placeholder for dynamic/any until supported
}
...
pub type BodyClause {
  ...
  Virtual(predicate: String, args: List(Part), outputs: List(String))
  ...
  BloomJoin(variable: String, clauses: List(BodyClause))
  BloomFilter(variable: String, filter: bloom.BloomFilter)
...
pub type SpeculativeResult {
  SpeculativeResult(state: DbState, datoms: List(fact.Datom))
}
