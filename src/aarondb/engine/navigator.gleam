...
import aarondb/shared/types.{
  type BodyClause, type Part, Aggregate, Bind, Cognitive, Filter, GroupBy,
  Limit, Negative, Offset, OrderBy, PageRank, Positive, ShortestPath, Similarity,
  ...
  case clause {
    Positive(#(e, _a, v)) -> {
      ...
      case e_bound, v_bound {
        ...
        // Fully bound: filter
        True, False -> 10
        // E bound: index lookup
        False, True -> 100
        // V bound: reverse index lookup
        False, False -> 1000
        // Unbound: scan
      }
    }
    Negative(_) -> 5000
    // Solve negatives late
    Filter(expr) -> {
...
fn clause_to_string(clause: BodyClause) -> String {
  ...
  case clause {
    ...
    Temporal(v, e, a, _, _, _) ->
      "Temporal(" <> v <> ", " <> part_to_string(e) <> ", " <> a <> ")"
    Cognitive(concept, context, _, var) ->
      "Cognitive(concept=" <> part_to_string(concept) <> ", context=" <> part_to_string(context) <> ", var=" <> var <> ")"
