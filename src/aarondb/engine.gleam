...
pub fn run(
  ...
  list.each(planned_clauses, fn(c) {
    case c {
      types.PageRank(_, edge, _, _, _)
      | types.CycleDetect(edge, _)
      | types.StronglyConnectedComponents(edge, _, _)
      | types.TopologicalSort(edge, _, _) -> {
...
fn solve_bloom_join(
  ...
  list.fold(clauses, #([ctx], None), fn(acc, c) {
    ...
    list.fold(contexts, #([], current_store), fn(inner_acc, inner_ctx) {
      let #(acc_ctxs, acc_store) = inner_acc
      let #(new_ctxs, clause_store) =
        solve_clause(db_state, c, inner_ctx, rules, as_of_tx, as_of_valid)
      #(
        list.append(acc_ctxs, new_ctxs),
        merge_optional_stores(acc_store, clause_store),
      )
...
fn solve_clause(
  ...
  case clause {
    ...
    types.Cognitive(concept, context, threshold, engram_var) -> #(
      solve_cognitive(
        db_state,
        concept,
        context,
    ...
    types.Temporal(var, entity, attr, start, end, basis) -> #(
      solve_temporal(db_state, var, entity, attr, start, end, basis, ctx),
      None,
    )
    ...
    types.PageRank(entity_var, edge, rank_var, d, iter) -> #(
      solve_pagerank(db_state, entity_var, edge, rank_var, d, iter, ctx),
      None,
    )
    ...
    fact.Columnar, filters -> {
      ...
      let cracking_pivots =
        list.filter_map(filters, fn(c) {
          case c {
            types.Filter(types.Gt(types.Var(v), types.Val(p)))
              if v == target_var
            -> Ok(p)
            types.Filter(types.Lt(types.Var(v), types.Val(p)))
              if v == target_var
            -> Ok(p)
            _ -> Error(Nil)
          }
...
pub fn pull(
  ...
  let id = case eid {
    ...
    fact.Lookup(#(a, v)) -> {
      index.get_entity_by_av(db_state.avet, a, v)
          ...
          case item {
                ...
                False -> {
                  ...
                  let results =
                    list.map(values, fn(v) {
                      case v {
                        fact.Ref(next_id) -> {
                          pull(db_state, fact.Uid(next_id), [
                            types.Wildcard,
                            types.Recursion(attr, depth - 1),
                          ])
                        }
                        fact.Int(next_id_int) -> {
                          pull(db_state, fact.Uid(fact.EntityId(next_id_int)), [
                            types.Wildcard,
                            types.Recursion(attr, depth - 1),
                          ])
                        }
                  ...
                  case results {
                    ...
                    [_, ..] ->
                      dict.insert(acc, attr, types.PullNestedMany(results))
                    [] -> acc
                  }
                }
              }
            ...
            types.Nested(name, sub_pattern) -> {
              ...
              case values {
                ...
                [fact.Int(sub_id)] -> {
                  let res =
                    pull(db_state, fact.Uid(fact.EntityId(sub_id)), sub_pattern)
                  dict.insert(acc, name, res)
                }
                [_, ..] -> {
                  let res_list =
                    list.map(values, fn(v) {
                      case v {
                        fact.Ref(eid) ->
                          pull(db_state, fact.Uid(eid), sub_pattern)
                        fact.Int(sub_id) ->
                          pull(
                            db_state,
                            fact.Uid(fact.EntityId(sub_id)),
                            sub_pattern,
                          )
                        _ -> types.PullSingle(v)
                      }
                  ...
                  case res_list {
                    ...
                    [_, ..] ->
                      dict.insert(acc, name, types.PullNestedMany(res_list))
                    _ -> acc
                  }
                }
                _ -> acc
...
fn solve_temporal(
  ...
  |> list.map(fn(d) { dict.insert(ctx, var, d.value) })
}
...
pub fn solve_cognitive(
  ...
  let active_concept = case concept_val {
  ...
  let active_context = case context_val {
